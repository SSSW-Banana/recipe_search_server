...
def fetch_clien_latest_data(ingredient):
    output = []
    igdDict = {'감자': 'basefoods%5B%5D=851', '삼겹살': 'basefoods%5B%5D=2489', '양파': 'basefoods%5B%5D=1639',
               '계란': 'basefoods%5B%5D=2781', \
               '당근': 'basefoods%5B%5D=1392', '양배추': 'basefoods%5B%5D=1625'}
...
ingredient = ["감자"]
#print(fetch_clien_latest_data(ingredient))

if __name__=='__main__':
    blog_data_dict = fetch_clien_latest_data(ingredient)
...
# 학습된 재료들과 재료들의 고유번호
# igdDict={'감자':'basefoods%5B%5D=851','삼겹살':'basefoods%5B%5D=2489','양파':'basefoods%5B%5D=1639','계란':'basefoods%5B%5D=2781',\
#     '당근':'basefoods%5B%5D=1392','양배추':'basefoods%5B%5D=1625'}
...
#     for j in range(1,13):
#         selector='#content > section > div.recipes > div > ul > li:nth-child('+str(j)+') > p > a > strong'
#         selector2= '#content > section > div.recipes > div > ul > li:nth-child('+str(j)+') > div.share_on > dl > dd:nth-child(5) > a'
#         title=soup.select_one(selector)
#         address=soup.select_one(selector2)
#         if not title :
#             break
